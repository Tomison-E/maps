...
class MapMouseEvent extends JsObjectWrapper<MapMouseEventJsImpl> {
  ...
  web.MouseEvent get originalEvent => jsObject.originalEvent;
...
class MapTouchEvent extends JsObjectWrapper<MapTouchEventJsImpl> {
  ...
  /// The DOM event which caused the map event.
  web.TouchEvent get originalEvent => jsObject.originalEvent;
  ...
  List<Point> get points =>
      jsObject.points.toDart.map((f) => Point.fromJsObject(f)).toList();

  ///  The geographical locations on the map corresponding to a
  ///  [touch event's `touches`](https://developer.mozilla.org/en-US/docs/Web/API/TouchEvent/touches) property.
  List<LngLat> get lngLats => jsObject.lngLats.toDart
